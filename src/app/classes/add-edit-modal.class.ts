...
import { ModalFormArray } from '../components/modal-form-array/types/modal-form-array.type';
...
export class AddEditModal {
  ...
  public setFormArrayValues(
    values: ModalFormArray[],
    formArray: FormArray
  ): void {
    values
      .slice(1, values.length)
      .map(() =>
        this.formBuilder.control('', [createTrimWhitespaceValidator()])
      )
      .forEach((control) => formArray.push(control));
  }

  public watchFormValueChanges(
    form: FormGroup,
