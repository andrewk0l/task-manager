...
export class UtilitiesUiService {
  private expandSidebar: WritableSignal<boolean> = signal(
    this.getInitialExpandSidebarValue
  );
  private showSidebarState: WritableSignal<boolean> = signal(
    this.shouldShowSidebarMenu(window.innerWidth)
  );
  ...
  constructor(@Inject(DOCUMENT) private readonly document: Document) {
    effect(() => {
      ...
      localStorage.setItem(
        LocalStorage.FRONTEND_MENTOR_EXPAND_SIDEBAR,
        JSON.stringify(expandSidebar)
      );
    });
  }
  ...
  public get getInitiallySelectedTheme(): boolean {
    try {
      return JSON.parse(
        localStorage.getItem(LocalStorage.FRONTEND_MENTOR_THEME) || 'false'
      );
    } catch {
      return false;
    }
  ...
  private get getInitialExpandSidebarValue(): boolean {
    try {
      return JSON.parse(
        localStorage.getItem(LocalStorage.FRONTEND_MENTOR_EXPAND_SIDEBAR) ||
          'true'
      );
    } catch {
      return true;
