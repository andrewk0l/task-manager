...
import { TaskModalForm } from './models/task-modal-form.model';
...
import { ModalFormArray } from '../modal-form-array/types/modal-form-array.type';
import {
  FormArray,
  FormControl,
  FormGroup,
  ...
  implements OnInit, AfterViewInit
{
  ...
  public dropdownValues: LabelValue[] = [];

  private initialFormValues!: Task;
  private newSubtasks: Subtask[] = [];
  ...
  public onSubmitForm(): void {
    ...
    const selectedBoardId = (this.storeBoardService.getSelectedBoard() as Board)
      .id;

    this.isAddMode
      ? this.handleAddTask(value, id, selectedBoardId)
      : this.handleUpdateTask(value, id, selectedBoardId);
  }

  public onSetNewSubtasks(newSubtasks: ModalFormArray[]): void {
    this.newSubtasks = newSubtasks.slice() as Subtask[];
  }
  ...
  private handleAddTask(
    formValues: TaskModalForm,
    id: string,
    selectedBoardId: string | null
  ): void {
    const subtasks = this.storeTaskService.createNewSubtasks(
      formValues.subtasks
    );

    this.storeTaskService.addNewTask(
      { ...formValues, id, subtasks },
      selectedBoardId
    );
  }
  ...
  private handleChangeColumn(taskId: string, status: string): void {
    if (this.initialFormValues.status === status) {
      return;
    }
  ...
  private setFormValues(): void {
    ...
    const { title, description, status, subtasks } = this.data
