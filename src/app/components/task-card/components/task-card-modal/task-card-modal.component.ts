...
export class TaskCardModalComponent implements OnInit {
  ...
  public selectedTask!: Signal<Task | null>;

  constructor(
    @Inject(MAT_DIALOG_DATA)
    public data: { task: Task; completedAmount: number }
  ) {}

  public ngOnInit(): void {
    this.dropdownValues = this.storeBoardService.getTaskStatus();
    this.selectedTask = this.storeTaskService.getSelectedTask(
      this.data.task.id
    );
  }
  ...
  public onDeleteTask(): void {
    this.dialog.open(TaskModalDeleteComponent, {
      data: { selectedTask: this.selectedTask() },
      enterAnimationDuration,
    });
  }
