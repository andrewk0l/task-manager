...
export class ColumnComponent {
  ...
  public onDrop(event: unknown) {
    const selectedBoardId = (this.storeBoardService.getSelectedBoard() as Board)
      .id;
    const { container, previousContainer, currentIndex, previousIndex } =
      event as CdkDragDrop<string[]>;

    if (previousContainer === container) {
      this.storeTaskService.moveTaskInExistingColumn(
        this.column,
        selectedBoardId,
        currentIndex,
        previousIndex
    ...
    this.storeTaskService.transferTaskInAnotherColumn(
      this.column,
      previousColumnId,
      selectedBoardId,
      currentIndex,
