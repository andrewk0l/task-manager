  ...
  implements OnInit, AfterViewInit
{
  ...
  private newColumns: Column[] = [];
  private initialFormValues!: BoardForm;

  constructor(@Inject(MAT_DIALOG_DATA) public data: { board: Board | null }) {
    super();
  }
  ...
  public onSubmitForm(): void {
    ...
    this.isAddMode
      ? this.handleAddBoard(name, id, columns)
      : this.handleUpdateBoard(name, id);
  }

  public onSetNewColumns(newColumns: ModalFormArray[]): void {
    this.newColumns = newColumns.slice() as Column[];
  }
  ...
  private setFormValues(): void {
    ...
    const { name, columns } = this.data.board as Board;
