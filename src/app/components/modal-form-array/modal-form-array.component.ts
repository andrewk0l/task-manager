...
import { ModalFormArray } from './types/modal-form-array.type';
import {
  ControlContainer,
  FormArray,
  FormBuilder,
...
export class ModalFormArrayComponent
  implements OnInit, AfterViewInit, OnDestroy
{
  @Output() itemsChangedEvent: EventEmitter<ModalFormArray[]> =
    new EventEmitter<ModalFormArray[]>();
  ...
  public ngOnInit(): void {
    ...
    this.parentFormGroup.addControl(
      this.controlName,
      this.formBuilder.array([this.getNewFormControl])
    );
    this.formArray = this.parentFormGroup.controls[
      this.controlName
    ] as FormArray;
  }
  ...
  private setAddTextButton(): void {
    this.addNewText = !this.subtaskMode
      ? '+ Add New Column'
      : '+ Add New Subtask';
  }
      ...
      .subscribe((formValues) => {
        this.items.update((previous) =>
          previous.map((item, index) => ({
            ...item,
            [this.getNewItemKey]: formValues[index],
          }))
        );
      });
  }
